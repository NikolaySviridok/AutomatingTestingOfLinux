...
def make_files():
    ...
    for i in range(data['count']):
        ...
        if ssh_checkout(f"{data['ip']}",
                        f"{data['user']}",
                        f"{data['password']}",
                        f"cd {data['folder_ext']
                              }; dd if=/dev/urandom of={filename} b"
...
def make_bad_arx():
    ssh_checkout(f"{data['ip']}",
                 f"{data['user']}",
                 f"{data['password']}",
                 f"cd {data['folder_in']}; 7z a {
                     data['folder_out']}/bad_arx -t{data['type']}",
                 "Everything is Ok")
    ssh_checkout(f"{data['ip']}",
                 f"{data['user']}",
                 f"{data['password']}",
...
def make_sub_folder():
    ...
    if not ssh_checkout(f"{data['ip']}",
                        ...
                        f" dd if=/dev/urandom of={testfilename} bs={
                            data['bs']} count=1 iflag=fullblock",
                        ''):
...
def stat_log():
    ...
    ssh_checkout(f"{data['ip']}",
                 f"{data['user']}",
                 f"{data['password']}",
                 f"echo 'time:{time} count:{data['count']} size;{
                     data['bs']} stat:{stat}' >> stat.txt",
                 '')
