...
class TestPositive:
    ...
    def test_step1(self, make_folders, make_files):
        result1 = ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                               f"cd {
                                   data['folder_in']}; 7z a -t{data['type']} {data['folder_out']}/arx2",
                               f"{data['valid']}")
    ...
    def test_step2(self, make_files):
        result1 = ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                               f"cd {data['folder_out']}; 7z e arx2.{
                                   data['type']} -o{data['folder_ext']} -y",
                               f"{data['valid']}")
    ...
    def test_step4(self):
        result1 = ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                               f"cd {data['folder_out']}; 7z x arx2.{
                                   data['type']} -o{data['folder_ext2']}",
                               f"{data['valid']}")
        assert result1, "test4 FAIL"

    def test_step6(self):
        assert ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                            f"cd {data['folder_out']}; 7z t arx2.{
                                data['type']}",
                            f"{data['valid']}"), "test6 FAIL"

    def test_step7(self):
        assert ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                            f"cd {data['folder_in']}; 7z u {
                                data['folder_out']}/arx2.{data['type']}",
                            f"{data['valid']}"), "test7 FAIL"

    def test_step8(self):
        assert ssh_checkout(f"{data['ip']}", f"{data['user']}", f"{data['password']}",
                            f"cd {data['folder_out']}; 7z d arx2.{
                                data['type']}",
    ...
    def test_step5(self, make_files, make_sub_folder):
        result2 = ssh_checkout(f"{data['ip']}",
                               f"{data['user']}",
                               f"{data['password']}",
                               f"cd {data['folder_in']}; 7z a -t{data['type']
                                                                 } {data['folder_out']}/arx2",
                               f"{data['valid']}")
